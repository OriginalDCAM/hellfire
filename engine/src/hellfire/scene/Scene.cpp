...
namespace hellfire {
    ...
    Entity *Scene::get_entity(const EntityID id) {
        const auto it = entities_.find(id);
        return it != entities_.end() ? it->second.get() : nullptr;
    }

    const Entity *Scene::get_entity(const EntityID id) const {
        const auto it = entities_.find(id);
        return it != entities_.end() ? it->second.get() : nullptr;
    }

    bool Scene::is_descendant(const EntityID potential_descendant, const EntityID potential_ancestor) {
    ...
    void Scene::set_parent(const EntityID child_id, const EntityID parent_id) {
        if (!entities_.contains(child_id)) return;
        if (parent_id != 0 && !entities_.contains(parent_id)) return;
        ...
        if (const auto it = parent_map_.find(child_id); it != parent_map_.end()) {
            const EntityID old_parent = it->second;
            auto &siblings = children_map_[old_parent];
            std::erase(siblings, child_id);
        } else {
            // Was a root entity
            std::erase(root_entities_, child_id);
        }
    ...
    void Scene::set_as_root(const EntityID entity_id) {
        set_parent(entity_id, 0);
    }

    EntityID Scene::get_parent(const EntityID entity_id) const {
        const auto it = parent_map_.find(entity_id);
        return it != parent_map_.end() ? it->second : 0;
    }

    bool Scene::has_parent(const EntityID entity_id) const {
        const auto it = parent_map_.find(entity_id);
        return it != parent_map_.end();
    }

    std::vector<EntityID> Scene::get_children(const EntityID parent_id) const {
    ...
    void Scene::set_default_camera(const EntityID camera_id) {
        const Entity *entity = get_entity(camera_id);
    ...
    void Scene::update_hierarchy(const EntityID entity_id, const float delta_time) {
        const Entity *entity = get_entity(entity_id);
        if (!entity) return;
        ...
        for (const EntityID child_id : get_children(entity_id)) {
            update_hierarchy(child_id, delta_time);
        }
    }

    void Scene::update_world_matrices_recursive(const EntityID entity_id, const glm::mat4 &parent_world) {
        const Entity *entity = get_entity(entity_id);
        if (!entity) return;
    ...
    void Scene::find_entities_recursive(const EntityID entity_id, const std::function<bool(Entity *)> &predicate,
