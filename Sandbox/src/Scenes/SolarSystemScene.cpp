...
hellfire::Entity *create_planet(const std::string &name, float scale, const glm::vec3 &position,
                              const std::shared_ptr<hellfire::Material> &material, float orbit_radius = 0.0f,
                              float orbit_speed = 0.0f,
                              const glm::vec3 &center = glm::vec3(0.0f)) {
    auto *planet = hellfire::Sphere::create(name, glm::vec3(1.0f), 64, 64);
    planet->transform()->set_position(position.x, position.y, position.z);
    planet->transform()->set_scale(1.0f * scale);
    planet->get_component<hellfire::RenderableComponent>()->set_material(material);
...
hellfire::Entity *create_planet_with_rotation(const std::string &name, float scale, const glm::vec3 &position,
                                            const std::shared_ptr<hellfire::Material> &material,
                                            float orbit_radius = 0.0f,
                                            float orbit_speed = 0.0f, float rotation_speed = 1.0f) {
    auto *planet = hellfire::Sphere::create(name);
    planet->transform()->set_position(position.x, position.y, position.z);
...
hellfire::Scene *load_solar_system_scene(const hellfire::AppInfo& window) {
    ...
    auto *world = new hellfire::Entity("Solar System World");
    scene->add_entity(world);

    // Setup lighting
    auto *sunlight = hellfire::PointLight::create("Sol Light", glm::vec3(0.0f), glm::vec3(1.0f), 1.2f, 350.0f);
    sunlight->get_component<hellfire::LightComponent>()->set_intensity(1.2f);
    world->add(sunlight);

    auto *sun_visual = hellfire::Sphere::create("Sol", glm::vec3(1.0f), 64, 64);
    sun_visual->transform()->set_position(0, 0, 0);
    sun_visual->transform()->set_scale(25.0f);
    sun_visual->get_component<hellfire::RenderableComponent>()->set_material(materials["SUN_MATERIAL"]);
    ...
    main_camera->get_component<hellfire::CameraComponent>()->look_at(glm::vec3(0.0f, 0.0f, 0.0f));
