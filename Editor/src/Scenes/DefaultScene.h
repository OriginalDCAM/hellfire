...
void setup_default_scene_with_default_entities(hellfire::Scene *scene) {
    ...
    for (int i = 0; i < 0; i++) {
        auto entity_id = hellfire::Cube::create(scene, "Cube_" + std::to_string(i),
                                                {.rotation = glm::vec3(-45.0f, 45.0f, 45.0f)});
        scene->get_entity(entity_id)->add_component<RotateScript>();

        if (i % 2 == 0) {
            auto child_entity_id = hellfire::Sphere::create(scene, "Sphere_Child_" + std::to_string(i), {
                                                                .color = glm::vec3(0.5, 0.2, 0.7),
    ...
    hellfire::Asset::load(scene, "assets/models/backpack/survival_guitar_backpack.glb", test_flags);
