...
namespace DCraft
{
    SceneManager::SceneManager() : active_scene_(nullptr), active_camera_(nullptr)
	{
	...
	void SceneManager::create_default_scene()
	{
		...
		auto* camera = new PerspectiveCamera(45.0f, 16.0f / 9.0f, 0.1f, 100.0f);
	...
	void SceneManager::destroy_object(Object3D* object)
	{
		...
		if (active_camera_ && objects_contains_camera_component(object, active_camera_))
		{
			active_camera_ = nullptr;
		}
